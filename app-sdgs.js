...
// ── LOGIN CHECK ──
// view mode (?view=) : ยอมรับทั้ง sdg_user (login รอบ 2) และ sdg_browse_user (login รอบ 1) — ดูอย่างเดียว
// edit/add mode      : ต้องมี sdg_user เท่านั้น (login รอบ 2)
const _params = new URLSearchParams(window.location.search);
const _isViewMode = !!_params.get('view');
const _dashUser = JSON.parse(sessionStorage.getItem('sdg_user') || 'null');
const _browseUser = JSON.parse(sessionStorage.getItem('sdg_browse_user') || 'null');

let currentUser;
if (_isViewMode) {
  // ดูอย่างเดียว — ใช้ session ไหนก็ได้ที่ล็อกอินแล้ว
  currentUser = _dashUser || _browseUser;
  if (!currentUser) {
    window.location.href = 'login.html';
  }
} else {
  // แก้ไข/เพิ่มโครงการ — ต้อง login รอบ 2 (sdg_user)
  currentUser = _dashUser;
  if (!currentUser) {
...
window.onload = function () {
  ...
  if (editId) {
    ...
    setTimeout(() => {
      ...
      // ปุ่มกลับ: ถ้ามาจากหน้าดูทั้งหมด (มีแค่ browse session) → กลับ browse.html
      //          ถ้าเป็นเจ้าของที่ login รอบ 2 → กลับ dashboard.html
      const backToBrowse = !_dashUser && _browseUser;
      ...
      if (actionBar) {
        actionBar.querySelectorAll('button').forEach(b => {
          ...
          if (t.includes('Dashboard')) {
            if (backToBrowse) {
              b.innerHTML = '← กลับหน้าดูโครงการทั้งหมด';
              b.setAttribute('onclick', "window.location.href='browse.html'");
            }
          } else if (!t.includes('พิมพ์')) {
